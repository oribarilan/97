...
Fires hardest when adding a request handler, RPC, or background job that will run in production with users depending on diagnosability. Fires lightly in MVPs, prototypes, internal dev tools, debugging endpoints, and one-off scripts where structured-logging, tracing, and SLO discipline are premature — prefer the simplest thing that works.
...
- `97/error-and-correctness-traps` overlaps on logging — it owns *content forbidden* (no secrets, no PII, no raw stack traces in prod). This skill owns *content shape* (structured key/value, request id, level, event name).
- `97/build-deploy-and-tooling` overlaps on logging — `12F/XI` (logs as event streams) owns *transport* (stdout/stderr aggregated by the platform). This skill owns *content shape*.
- `97/security-and-trust-boundaries` has precedence on what fields are safe to emit. If a log line might contain a token, password, or PII, that skill decides; this skill decides everything else about the line.
- `superpowers/systematic-debugging` runs first when the trigger is "diagnose this active production incident." This skill prevents under-instrumented code at write time.

## Trap checks by domain

Find the sub-section that matches what you're writing.

### Logs

A free-form log line in production is unsearchable text; a structured log line is queryable data.

1. **Structured, not free-form.** Log as JSON or another key/value format the platform parses. Keys: `timestamp`, `level`, `event` (a short stable name like `user_login_failed`), plus the relevant context fields (`request_id`, `user_id` when not sensitive, `route`, `duration_ms`, `status`). Concrete trap: `logger.info(f"user {user.id} logged in via {provider} at {ts}")` is unsearchable; `logger.info("user_login", user_id=user.id, provider=provider)` is queryable. *(`OTel/StructuredLogs`.)*
...
| "I'll add the full URL path as a label." | Same trap — `/users/12345` and `/users/12346` are different series. Use the route template (`/users/:id`), not the realized path. (`OE/CardinalityDiscipline`) |
...
If any box that applies to your change is unchecked, you are not done — you are mid-instrumentation.
