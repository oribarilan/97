...
Fires hardest when code will reach real users in production. Fires lightly in MVPs, prototypes, internal dev tools, and one-off scripts where the architecture is not yet settled — prefer the simplest thing that works.
...
| "Failed call → just retry in a loop until it works." | Retry without backoff and a cap is a self-DDoS the moment the service recovers. Backoff + jitter + ceiling, and require idempotency on the server. (97/41) |
