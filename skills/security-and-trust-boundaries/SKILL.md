...
| "Just put the API key in the config file for now — we'll move it before launch." / *finding a credential-shaped string literal in a file you're editing* | Once it's in git, it's leaked. Rotate the moment it's pushed. Surface and address even in prototypes — this trap has no stage exemption. (97/9) |
...
- [ ] **Crypto:** passwords use a password-hash (bcrypt/scrypt/argon2id) at a documented cost; security tokens use the cryptographic RNG (not `Math.random`/`random.random`); no hand-rolled crypto, no hardcoded IVs. Adjacent code with these traps is surfaced, not silently rewritten.
